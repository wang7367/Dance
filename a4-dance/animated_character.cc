                                          ...
                                          const Matrix4 &view_matrix, const Matrix4 &proj_matrix)
{
    ...
    */
    ...
    if (bone_name == "lhipjoint" || bone_name == "rhipjoint") {
        quick_shapes_.DrawLineSegment(ctm, view_matrix, proj_matrix, Color(0, 0, 0), Point3(0.0, 0.0, 0.0), Point3(0.0, 0.0, 0.0) + skeleton_.BoneDirectionAndLength(bone_name), 0.01);
    }
    ...
    if (bone_name == "lowerback") {
        Matrix4 S = Matrix4::Scale(Vector3(0.18, 0.3, 0.18));
        Matrix4 T = Matrix4::Translation(Vector3(0, -0.8, 0.5));
        Matrix4 R = Matrix4::RotationX(0.523);
        quick_shapes_.DrawSphere(ctm*R*S*T, view_matrix, proj_matrix, Color(199.0/255.0, 12.0/255.0, 12.0/255.0));
        //quick_shapes_.DrawLineSegment(ctm, view_matrix, proj_matrix, Color(1, 1, 1), Point3(0.0, 0.0, 0.0), Point3(0.0, 0.0, 0.0) + skeleton_.BoneDirectionAndLength(bone_name), 0.01);
    }
    if (bone_name == "upperback") {
        Matrix4 S = Matrix4::Scale(Vector3(0.12, 0.12, 0.12));
        Matrix4 T = Matrix4::Translation(Vector3(0, -0.2, 0));
        quick_shapes_.DrawSphere(ctm * S * T, view_matrix, proj_matrix, Color(199.0 / 255.0, 12.0 / 255.0, 12.0 / 255.0));
    }
    if (bone_name == "thorax") {
        Matrix4 S = Matrix4::Scale(Vector3(0.12, 0.12, 0.12));
        Matrix4 T = Matrix4::Translation(Vector3(0, 0.6, 0));
        quick_shapes_.DrawSphere(ctm * S * T, view_matrix, proj_matrix, Color(199.0 / 255.0, 12.0 / 255.0, 12.0 / 255.0));
    }
    ...
    if (bone_name == "head") {
        Matrix4 S = Matrix4::Scale(Vector3(0.09, 0.18, 0.09));
        Matrix4 T = Matrix4::Translation(Vector3(0, 0, -0.8));
        Matrix4 R = Matrix4::RotationX(2.356);
        quick_shapes_.DrawSphere(ctm * R * S * T, view_matrix, proj_matrix, Color(199.0 / 255.0, 12.0 / 255.0, 12.0 / 255.0));
        Matrix4 S1 = Matrix4::Scale(Vector3(0.03, 0.03, 0.03));
        Matrix4 T1 = Matrix4::Translation(Vector3(1.5, 3.0, 3.0));
        quick_shapes_.DrawSphere(ctm  * S1 * T1, view_matrix, proj_matrix, Color(0, 0, 0));
        Matrix4 S2 = Matrix4::Scale(Vector3(0.03, 0.03, 0.03));
        Matrix4 T2 = Matrix4::Translation(Vector3(-1.5, 3.0, 3.0));
        quick_shapes_.DrawSphere(ctm * S2 * T2, view_matrix, proj_matrix, Color(0, 0, 0));

        quick_shapes_.DrawLineSegment(ctm, view_matrix, proj_matrix, Color(0, 0, 0), Point3(-0.045, 0.1, 0.05), Point3(-0.045, 0.25, 0.05), 0.01);
        quick_shapes_.DrawLineSegment(ctm, view_matrix, proj_matrix, Color(0, 0, 0), Point3(0.045, 0.1, 0.05), Point3(0.045, 0.25, 0.05), 0.01);
    }
